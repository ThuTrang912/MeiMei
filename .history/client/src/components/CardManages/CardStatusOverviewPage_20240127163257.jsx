...
const CardStatusOverviewPage = () => {
    ...
    useEffect(() => {
        const fetchCardStatus = async () => {
            try {
                const response = await fetch(`http://${API_BASE_URL}:8000/api/manager/status`);
            ...
            <div className='w-full'>
                ...
                {cardStatusList.map((item, index) => (
                    <li key={index} className='flex items-center justify-around w-full text-xl text-center border border-[#36735B] rounded-xl'>
                        <p className='h-full w-full py-2 border-r border-r-[#36735B] border-b-[#36735B] text-[#36735B] font-medium'>{item.id_card}</p>
