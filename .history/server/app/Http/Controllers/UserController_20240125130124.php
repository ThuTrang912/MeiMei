...
class UserController extends Controller
{
    ...
    public function isRegistrationAllowed(string $id)
    {
        ...
        if ($manager) {
            ...
            $registrationAllowed = $manager->registration_allowed;
        }
    ...
    public function store(Request $request)
    {
        ...
        //viet them thang id card do vao user
        $idCard = $request->input('id_card');

        // Check if a user with the given ID card already exists
        $existingManager = Manager::where('id_card', $idCard)->first();

        if ($existingManager) {
            // Update the existing user's information
            $existingManager->update($request->all());
            $user = $existingUser;
        } else {
            // Create a new user
